...
# dont think we need this anymore since we're using symlinks now for logins/team-names
#def publish_htaccess():
#    print('Writing htaccess file...')
#    try:
#        with open(settings.HTACCESS_PREAMBLE_FILE, 'r') as preamble_file:
#            preamble = preamble_file.read()
#        with open(settings.HTACCESS_FILE, 'w') as htaccess_file:
#            htaccess_file.write('# DO NOT EDIT - this is generated by spoilr from %s and information in the hunt database\n\n' % (settings.HTACCESS_PREAMBLE_FILE))
#            htaccess_file.write(preamble)
#            htaccess_file.write('# Below here, one stanza per team, redirecting them by username to their team directory\n')
#            for team in Team.objects.all():
#                htaccess_file.write('''
#RewriteCond %%{LA-U:REMOTE_USER} %s
#RewriteRule ^(.*)$ /teams/%s/$1 [NS,L]
...
def prestart_team(team, suffix=None):
    ...
    if not os.path.exists(team_path):
        try:
            ...
            return False
    if not os.path.lexists(team_symlink):
        try:
            os.symlink(team_path, team_symlink)
        except OSError as e:
            logger.error('Failed to create team symlink from %s to %s: %s', team_path, team_symlink, str(e))
            return False
    try:
